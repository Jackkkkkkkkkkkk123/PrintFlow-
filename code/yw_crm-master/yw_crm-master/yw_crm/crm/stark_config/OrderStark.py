...
class OrderStark(Permissions,BaseStark):
    ...
    def get_urls(self):
        urlpatterns = [
            re_path(r'customer/order/(?P<customer_id>\d+)/$', self.wrapper(self.changelist_view), name=self.get_list_url_name),
            re_path(r'add/(?P<customer_id>\d+)/$', self.wrapper(self.add_view), name=self.get_add_url_name),
            re_path(r'(?P<customer_id>\d+)/(?P<pk>\d+)/change/$', self.wrapper(self.change_view),name=self.get_edit_url_name),
            re_path(r'(?P<customer_id>\d+)/(?P<pk>\d+)/del/$', self.wrapper(self.del_view), name=self.get_del_url_name),
        ]
